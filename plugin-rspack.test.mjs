...
import { HtmlRspackPlugin as HtmlWebpackPlugin, sources } from '@rspack/core';
import {
  WEBPACK_OUTPUT_DIR,
  createWebpackConfig,
  webpackCompile,
...
describe('CspHtmlRspackPlugin', () => {
  ...
  describe('Adding sha and nonce checksums', () => {
    it('preserves inline script and style bytes with the default process function', () =>
      new Promise((resolve, reject) => {
        const done = (error) => (error ? reject(error) : resolve());
        const inlineScript =
  ...
  describe('HTML parsing', () => {
    it.skip("doesn't encode escaped HTML entities", () =>
      new Promise((resolve, reject) => {
        const done = (error) => (error ? reject(error) : resolve());
        const config = createWebpackConfig([
          new HtmlWebpackPlugin({
    ...
    it.skip('honors xhtml mode if set on the html-webpack-plugin instance', () =>
      new Promise((resolve, reject) => {
        const done = (error) => (error ? reject(error) : resolve());
        const config = createWebpackConfig([
          new HtmlWebpackPlugin({
