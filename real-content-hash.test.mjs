...
function createUnavailableRealContentHashPlugin() {
  return {
    apply(compiler) {
      ['rspack', 'webpack'].forEach((namespace) => {
        const bundlerNamespace = Object.create(compiler[namespace]);
        bundlerNamespace.optimize = {
          ...compiler[namespace].optimize,
          RealContentHashPlugin: undefined,
        };
        Reflect.set(compiler, namespace, bundlerNamespace);
      });
    },
  };
}
...
describe('CSP real content hash integration', () => {
  ...
  it('registers generated CSP hashes and recomputes them from final HTML', () =>
  ...
  it('skips real content hash hooks and metadata when disabled', () =>
    new Promise((resolve, reject) => {
      const done = (error) => (error ? reject(error) : resolve());
      hookFacade = installRealContentHashHookFacade();
      const inspectionPlugin = new InspectionPlugin();
      const cspPlugin = new CspHtmlRspackPlugin({}, PLUGIN_OPTIONS);
      const getHashRecords = rs.spyOn(cspPlugin, 'getHashRecords');
      const recordRealContentHashes = rs.spyOn(
        cspPlugin,
        'recordRealContentHashes'
      );
      const oldHash = digest(
        'sha384',
        'loadChunk("async-old-content-hash.js")'
      );
  ...
  it('prefers the Rspack namespace for real content hash hooks', () =>
    new Promise((resolve, reject) => {
      const done = (error) => (error ? reject(error) : resolve());
  ...
  it('falls back to the webpack namespace for real content hash hooks', () =>
    new Promise((resolve, reject) => {
      const done = (error) => (error ? reject(error) : resolve());
      const webpackFacade = createRealContentHashPluginFacade();
  ...
  it('matches shared hashes to their corresponding HTML output', () =>
    new Promise((resolve, reject) => {
      const done = (error) => (error ? reject(error) : resolve());
      hookFacade = installRealContentHashHookFacade();
      ...
      compileWithPlugins(
        ...
        () => {
          ...
          expect(hookFacade.getUpdateHash()(finalAssets, oldHash)).toBe(
            newHash
          );
          done();
        }
  ...
  it('registers hashes on HTML filenames with contenthash placeholders', () =>
    new Promise((resolve, reject) => {
      const done = (error) => (error ? reject(error) : resolve());
      hookFacade = installRealContentHashHookFacade();
  ...
  it('keeps existing behavior when the RealContentHash hook is unavailable', () =>
    new Promise((resolve, reject) => {
      const done = (error) => (error ? reject(error) : resolve());
      const inspectionPlugin = new InspectionPlugin();
      const oldHash = digest(
      ...
      compileWithPlugins(
        [
          new HtmlRspackPlugin({ filename: 'index.html', template: TEMPLATE }),
          createUnavailableRealContentHashPlugin(),
          new CspHtmlRspackPlugin({}, PLUGIN_OPTIONS),
          inspectionPlugin,
        ],
        (csps) => {
  ...
  it('does not register hashes emitted only by a custom processFn', () =>
    new Promise((resolve, reject) => {
      const done = (error) => (error ? reject(error) : resolve());
      hookFacade = installRealContentHashHookFacade();
      const inspectionPlugin = new InspectionPlugin();
